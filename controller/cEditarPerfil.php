...
define("OBLIGATORIO", 1);//Creo una constante y le asigno un 1.
    ...
    if(isset($_REQUEST["cambiar"])){//Si el usuario pulsa el botón de cambiar.
        $error=validacionFormularios::comprobarAlfabetico($_REQUEST["descUsuario"], 255, 3, OBLIGATORIO);//Compruebo que el campo descripción del usuario haya introducido datos válidos.
    ...
    if($entradaOk){//Si la entrada es Ok.
        ...
        $_SESSION["usuarioDAW203LoginLogoffMulticapa"]=$oUsuario;//Almaceno en la sesión la variable $oUsuario el resultado de la consulta.
    ...
    $fechaHoraUltimaActual = $_SESSION['usuarioDAW203LoginLogoffMulticapa']->getFechaHoraUltimaConexion();
